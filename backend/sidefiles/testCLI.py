import testLIDS as lids
import os
import testAlerts as alert
import testLIDSD as lidsd
import testPCAP as pcap
if __name__ == '__main__':
    try:

        file = os.path.join(os.path.dirname(__file__), './config_file.xml') 
        ...
        while True:
            try:
                ...
                if user.lower() == 'help':
                    ...
                    if exp.lower() == 'xml':
                        lidsd.export_alerts('xml')
                    elif exp.lower() == 'json':
                        lidsd.export_alerts('json')
                    elif exp.lower() == 'csv':
                        lidsd.export_alerts('csv')
                    else:
